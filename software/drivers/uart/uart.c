...
/* Pass a single character to OpenSBI via an environment call */
int uart_putc(char c) {
    register unsigned long a0 asm("a0") = (unsigned long)c;
    register unsigned long a7 asm("a7") = (unsigned long)0x01; // SBI Console Putchar Extension
...
/* Print a null-terminated string, automatically fixing newlines */
