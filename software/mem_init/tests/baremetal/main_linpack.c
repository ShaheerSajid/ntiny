...
int main()
    ...
    while (1)
        ...
        if ((MEM_T)malloc_arg!=memreq || (mempool=malloc(malloc_arg))==NULL)
            {
        		ee_printf("Not enough memory available for given array size.\n");
			}
        ee_printf("LINPACK benchmark, %s precision.\n",PREC);
        ee_printf("Machine precision:  %d digits.\n",BASE10DIG);
        ee_printf("Array size %d X %d.\n",arsize,arsize);
        ee_printf("Memory required:  %ldK.\n",(memreq+512L)>>10);
        ee_printf("Average rolled and unrolled performance:\n\n");
        ee_printf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS\n");
        ee_printf("----------------------------------------------------\n");
        nreps=1;
 ...
 REAL linpack(long nreps,int arsize)
    ...
    if (toverhead<0.)
        toverhead=0.;
    ee_printf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f\n",
