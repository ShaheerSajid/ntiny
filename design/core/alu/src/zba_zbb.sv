...
// ── CLZ (Count Leading Zeros) via lzc module ────────────────────────────
wire [4:0] clz_cnt;
wire       clz_zero;
lzc #(.WIDTH(32)) clz_inst (
	.a_i    (in1_i),
	.cnt_o  (clz_cnt),
	.zero_o (clz_zero)
);
assign out_clz_o = clz_zero ? 32'd32 : {27'd0, clz_cnt};

// ── CTZ (Count Trailing Zeros) via lzc on bit-reversed input ────────────
wire [31:0] in1_rev;
genvar g;
generate
	for (g = 0; g < 32; g = g + 1)
		assign in1_rev[g] = in1_i[31 - g];
endgenerate

wire [4:0] ctz_cnt;
wire       ctz_zero;
lzc #(.WIDTH(32)) ctz_inst (
	.a_i    (in1_rev),
	.cnt_o  (ctz_cnt),
	.zero_o (ctz_zero)
);
assign out_ctz_o = ctz_zero ? 32'd32 : {27'd0, ctz_cnt};
/****************************************adder tree *********************/
